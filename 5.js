...
function ode() {
	...
	psy = parseFloat(window.document.getElementById("idp").value);
	iu = parseInt(window.document.getElementById("idu").value);
	an = 100, ant = 10, am = 5;
...
g = function (y, t) {
	...
	if ((tau * psy) != 0) {
		v[0] = y[1];
		v[1] = -(tau + psy) / (tau * psy) * y[1] - 1 / (tau * psy) * y[0] + k / tau * u;
	}
	else {
		v[0] = -1 / (tau + psy) * y[0] + psy * k / (tau + psy) * u;
	}
	return v;
}
function data() {
